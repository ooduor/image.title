...
for idx, (_curr, _next) in enumerate(zip(contours[::],contours[1::])):
    ...
    if (ny-cy) > (nh+ch)*2: # next is greater than current...
        ...
        cv2.imwrite('article_{}inbig.png'.format(idx), article_mask)
        article_complete = True
    elif (ny-cy) < (nh+ch)*2 and (ny-cy) > 0: # next if not greater... but just small
        ...
        if len(contours) == (idx+2): # we are on last article, it's always greater by 2 instead of one. Nkt!
            ...
            cv2.imwrite('article_{}insmalllast.png'.format(idx), article_mask)
    ...
    elif (ny-cy) < (nh+ch)*2 and (ny-cy) < 0: # next if not greater... must be invalid
